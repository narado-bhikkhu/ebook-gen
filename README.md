...
### Generate high-quality single PDF from paged data
...
./ebook-gen \
  ...
  -format pdf \
  -quality \
  -output high-quality-dictionary.pdf
```

The `-quality` flag generates PDFs with higher DPI (150 instead of 72) and removes the low-quality flag, resulting in sharper text and images. When used with `-paged`, it creates a single PDF file instead of splitting into multiple parts for size reasons.

**Note**: The high-quality PDF generation properly handles Tibetan Unicode characters in definitions by applying the appropriate font styling.
...
-paged
    Read per-page JSON files from the 'paged' subdirectory and treat each file as one ebook page

-format string
    Output format: epub|pdf|html (when empty defaults to epub; if --paged and empty defaults to pdf)

-quality
    Generate high-quality PDF with higher DPI (150 instead of 72) and no low-quality compression
